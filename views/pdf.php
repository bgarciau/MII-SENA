...
// HEADER ----------------------------------------------------------------------------------------
        ...
        $pdf->Write(2, 'HOJA DE VIDA APRENDICES EN PROGRAMAS DE FORMACIÓN TITULADA');
        $pdf->Ln();
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(8, 'F04-9124-002/ 06-10');
        $pdf->Ln();
        $pdf->Write(5, 'Ejecución de la Formación Profesional');
        $pdf->Ln();
...
// INFORMACION GENERAL DEL APRENDIZ--------------------------------------------------------------
        ...
        $pdf->Ln();
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(5, 'Información personal del aprendiz. Usted como empleador podrá solicitar ampliación de esta.');
        $pdf->Ln();
...
// FORMACION ACADEMICA--------------------------------------------------------------
        ...
        foreach ($estudios as $estudio) {
            ...
            $pdf->Ln();
            $pdf->Cell(60, 5, 'INSTITUCIÓN EDUCATIVA', 1, 0, false);
        ...
        $pdf->Ln();
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(5, 'Si usted ha realizado estudios de nivel superior como Técnico TC, Tecnólogo TL, Especialización Tecnológica TE, Universitaria UN, Especialización ES, ');
        $pdf->Write(4, 'Maestría MG, Doctorado DOC, relaciónelos a continuación');
        $pdf->SetFont('DejaVuBold', '', 9);
        ...
        foreach ($ficha as $datosFicha) {
            ...
            foreach ($programa as $datosPrograma) {
                ...
                foreach ($centro as $datosCentro) {
                    $pdf->SetFont('DejaVu', '', 8);
                    $pdf->Write(5, 'Información relevante del programa de formación, para más información podrá contactar al Coordinador Académico');
                    $pdf->Ln();
                    $pdf->SetFont('DejaVuBold', '', 9);
                    ...
                    $pdf->Ln();
                    $pdf->Cell(60, 5, 'OCUPACIONES A DESEMPEÑAR', 1, 0, false);
...
// 4 FIRMA DEL APRENDIZ--------------------------------------------------------------
        ...
        $pdf->Write(3, 'PRÁCTICA DESEMPEÑANDOME EN UNA EMPRESA A TRAVÉS DEL CONTRATO DE APRENDIZAJE. PARA TODOS LOS EFECTOS LEGALES,');
...
// FIRMA FUNCIONARIO PROMOCION Y RELACIONAMIENTO CORPORATIVO SENA--------------------------------------------------------------
        ...
        $pdf->Ln();
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(5, ' Información del funcionario encargado en el Centro de Formación. Contacte para la legalización del contrato de aprendizaje');
        $pdf->Ln();
        $pdf->SetFont('DejaVuBold', '', 9);
        $pdf->Cell(60, 5, 'NOMBRE FUNCIONARIO', 1, 0, false);
        $pdf->Cell(135, 5, 'Carlos Ruiz', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'TELEFONO', 1, 0, false);
        $pdf->Cell(135, 5, '3124564567', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'CORREO ELECTRONICO', 1, 0, false);
        $pdf->Cell(135, 5, 'carlosruiz@sena.com', 1, 0, false);
        $pdf->Ln(8);
        // ----------------------------------------------------------------------------------------------
// INFORMACION SERVICIO NACIONAL DE APRENDIZAJE-------------------------------------------------------------
        ...
        $pdf->Cell(135, 5, '899.999034-1', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'CENTRO FORMACION', 1, 0, false);
        $pdf->Cell(135, 5, 'Centro de Automatización Industrial', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'REPRESENTANTE LEGAL', 1, 0, false);
        $pdf->Cell(135, 5, 'Jorge Eduardo Londoño ulloa', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'CORREO ELECTRONICO', 1, 0, false);
        $pdf->Cell(135, 5, 'JorgeLondoño@sena.com', 1, 0, false);
        $pdf->Ln();
        $pdf->Cell(60, 5, 'TELEFONO', 1, 0, false);
        $pdf->Cell(135, 5, '3214535369', 1, 0, false);
        $pdf->Ln(8);
        // ----------------------------------------------------------------------------------------------
// OBSERVACIONES DEL JEFE DE RECURSOS HUMANOS Y/O CONTRATOS-------------------------------------------------------------
        ...
        $pdf->Ln(65);
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(2, 'Marque con una x la decisión de contratar al aprendiz. Seleccionado: _____ No Seleccionado: ____ ');
        $pdf->Ln(10);
        $pdf->SetFont('DejaVu', '', 10);
        $pdf->Write(2, 'Ciudad y Fecha de Diligenciamiento: _____________________________FIRMA: _______________________________');
        $pdf->Ln(10);
        $pdf->SetFont('DejaVu', '', 8);
        $pdf->Write(5, 'Solicitamos a la empresa imprimir y suministrar copia de este documento una vez realizada la evaluación del aprendiz que será remitida a la oficina de
